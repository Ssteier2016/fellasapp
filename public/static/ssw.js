...
self.addEventListener('fetch', (event) => {
    ...
    // Asumimos que las APIs de Showroom +Roma serán externas y no deben ser cacheadas.
    if (event.request.url.includes('api.tutienda.com') || event.request.url.includes('google.com')) {
        return; 
    }
    ...
    event.respondWith(
        caches.match(event.request)
            .then((response) => {
                ...
                // Si no está, va a la red y luego lo añade al caché para uso futuro (si es HTML, CSS o JS)
...
self.addEventListener('push', event => {
  ...
  self.registration.showNotification(data.title, {
    body: data.body,
    icon: '/static/icon3.png', // Usar el ícono en static
    data: { url: data.url }
  });
});
